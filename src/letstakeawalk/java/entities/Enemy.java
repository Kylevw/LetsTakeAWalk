...
public class Enemy extends Entity {
    ...
    public void timerTaskHandler() {
        ...
        if (!attackTimer.isComplete()) {
            ...
            if (getPlayerDistance() <= getAttackDistance() && attackTimer.isComplete()) {
                attackTimer.start();
                startAttackAI();
            } else if (getPlayerDistance() <= getSightDistance() && attackTimer.isComplete()) {
                targetAI();
        ...
        if (!invulTimer.isComplete()) {
            setImage(getImageProvider().getTintedImage((BufferedImage) getAnimator().getCurrentImage(), new Color(255, 0, 0, 100)));
        }
    ...
    public double getPlayerDistance() {
        if (player != null) {
            return 2.4;
//            return TrigonometryCalculator.getHypotenuse(getPosition().x, getPosition().y, player.getPosition().x, player.getPosition().y);
        } else {
    ...
    public void draw(Graphics2D graphics) {
        ...
        if (drawBoundary()) {
            graphics.drawOval(getPosition().x - sightDistance, getPosition().y - sightDistance, sightDistance * 2, sightDistance * 2);
        }
        graphics.setColor(Color.RED);
        if (drawBoundary()) {
            graphics.drawOval(getPosition().x - attackDistance, getPosition().y - attackDistance, attackDistance * 2, attackDistance * 2);
        }
        super.draw(graphics);
    }
    ...
    public void accelerateKnockbackVelocity(int x, int y) {
        if (invulTimer.isComplete()) {
            super.accelerateKnockbackVelocity(x, y);
        }
    }
